...
def create_user(request: Request, user_create_schema: UserCreateSchema, session: Session = Depends(get_session)):
    """Criar um usuário"""
...
def login(request: Request, login_schema: LoginSchema, session: Session = Depends(get_session)):
    """Rota para login"""
...
def recover_password(request: Request, email: str, session: Session = Depends(get_session)):
...
@limiter.limit("5/minute", per_method=True)
def reset_password(request: Request, body: NewPassword, session: Session = Depends(get_session)):
